...
class BeerControllerTest {
    ...
    void getBeerById() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/beer/{beerId}", UUID.randomUUID().toString())
                .param("isCold", "yes") // ###RestDoc This is not a real parameter for this call but does //document how a request parameter is documented with ###RestDoc
                .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andDo(document("v1/beer-get", //// ###RestDoc
                        pathParameters(
                                parameterWithName("beerId").description("UUID of Desired beer to get.")
                        ),
                        requestParameters(
                                parameterWithName("isCold").description("Is The Beer Cold Query parameter.")
                        ),
                        responseFields(
